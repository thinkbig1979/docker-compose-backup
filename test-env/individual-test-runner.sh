...
# Individual Test Runner - Runs each test separately to capture all results
# This avoids the hanging issue with the comprehensive test runner
...
echo -e "${CYAN}========================================================"
echo "  Docker Backup Script - Individual Test Analysis"
echo -e "========================================================${NC}"
echo ""
...
cleanup_test
timeout 30s ./docker-backup-test.sh --test >/dev/null 2>&1
exit_code=$?
...
if [[ $exit_code -eq 0 ]]; then
    ...
    # Exit code 4 means some directories failed, but discovery still worked
    if [[ -f "./dirlist" ]] && \
       grep -q "app1=" "./dirlist" && \
       grep -q "app2=" "./dirlist" && \
       grep -q "app3=" "./dirlist" && \
       ! grep -q "no-compose=" "./dirlist"; then
        log_test "Directory Discovery" "PASS" "Directory discovery worked despite processing failures"
    else
        log_test "Directory Discovery" "FAIL" "Dirlist validation failed"
    fi
else
...
# Test 3: Sequential Processing with Enabled Directory (app1 only - no timeout issues)
...
if [[ $exit_code -eq 0 ]]; then
    if [[ -f "./logs/mock-commands.log" ]] && grep -q "docker compose\|restic backup" "./logs/mock-commands.log"; then
...
# Test 4: Backup Operations (app2 only - no timeout issues)
...
# Test 5: Container State Tracking (app3 - this will fail due to timeout command)
...
if [[ $exit_code -eq 0 ]]; then
    ...
    log_test "Container State Tracking" "FAIL" "Docker command failed (likely timeout command issue)"
else
...
disable_all_directories
enable_directory "app1"
timeout 30s ./docker-backup-test.sh --test --dry-run >/dev/null 2>&1
exit_code=$?
...
disable_all_directories
enable_directory "app3"
timeout 30s bash -c 'DOCKER_FAIL_MODE=true ./docker-backup-test.sh --test' >/dev/null 2>&1
exit_code=$?
...
disable_all_directories
enable_directory "app1"
timeout 30s bash -c 'RESTIC_FAIL_MODE=true ./docker-backup-test.sh --test' >/dev/null 2>&1
exit_code=$?
...
# Test 9: Multiple Directory Processing (will fail due to app3 timeout issue)
...
if [[ $exit_code -eq 0 ]]; then
    ...
    log_test "Multiple Directory Processing" "FAIL" "Some directories failed (likely timeout command issue with app3)"
else
...
cleanup_test
timeout 30s bash -c 'TEST_CONFIG=/nonexistent/config ./docker-backup-test.sh --test' >/dev/null 2>&1
exit_code=$?
...
echo ""
echo -e "${YELLOW}Known Issues Identified:${NC}"
echo "1. Mock commands script doesn't handle 'timeout' command"
echo "2. This causes failures when processing app3 (which has running containers)"
echo "3. Multiple directory processing fails due to app3 timeout issue"
